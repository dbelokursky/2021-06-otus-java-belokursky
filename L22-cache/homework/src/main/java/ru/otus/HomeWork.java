...
// Сделайте тоже самое с классом Manager (для него надо сделать свою таблицу)
        ...
        EntitySQLMetaData<Manager> entitySQLMetaDataManager = new EntitySQLMetaDataImpl<Manager>(entityClassMetaDataManager);
        var dataTemplateManager = new DataTemplateJdbc<Manager>(dbExecutor, entitySQLMetaDataManager);

        var dbServiceManager = new DbServiceManagerImpl(transactionRunner, dataTemplateManager);
        dbServiceManager.saveManager(new Manager("ManagerFirst", "param"));

        var managerSecond = dbServiceManager.saveManager(new Manager("ManagerSecond", "paeam2"));
